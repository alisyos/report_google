...
export async function GET(request: Request) {
  ...
  try {
    const report = await getCampaignPerformanceReport(startDate, endDate);
    
    console.log('API 응답 원본 데이터 예시:', report.length > 0 ? JSON.stringify(report[0], null, 2) : '데이터 없음');
    
    // 마이크로 단위 비용을 실제 화폐 단위로 변환
    const formattedReport = report.map((item: any) => {
      const costMicros = item.metrics?.cost_micros;
      console.log(`비용 마이크로 값: ${costMicros}, 타입: ${typeof costMicros}`);
    ...
    console.error('캠페인 리포트 API 오류:', error.message);
    return NextResponse.json(
      { error: error.message || '캠페인 리포트를 가져오는 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
} 